 ...
 * @property-write DateTimeZone|string $timezone1 the current timezone
 * @property-write DateTimeZone|string $tz1 alias of timezone
...
class Carbon extends DateTime
{
     ...
     * @return self
     */
    public function setTime($hour, $minute, $second = 0, $milsecond = 0)
    {
    ...
    public function endOfHour()
    {
        $hour = $this->hour + 1;
        if ($hour != 24) {
            return $this->hour($hour)->minute(0)->second(0);
        }
        return $this->addDay()->hour(0)->minute(0)->second(0);
    }
    ...
    {
        $dt = ($dt === null) ? static::now($this->tz) : $dt;
        [$sign, $years, $months] = explode(':', $this->diff($dt)->format('%r:%y:%m'));
    ...
    {
        $dt = ($dt === null) ? static::now($this->tz) : $dt;
        [$sign, $days, $hours, $minutes, $seconds] = explode(':', $this->diff($dt)->format('%r:%a:%h:%i:%s'));
